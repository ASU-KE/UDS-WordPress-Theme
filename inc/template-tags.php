	...
	function uds_wp_posted_on() {
		...
		$posted_on   = apply_filters(
			...
			sprintf(
				...
				esc_html_x( 'Posted on', 'post date', 'uds-wordpress' ),
		...
		$byline      = apply_filters(
			...
			sprintf(
				...
				$posted_on ? esc_html_x( 'by', 'post author', 'uds-wordpress' ) : esc_html_x( 'Posted by', 'post author', 'uds-wordpress' ),
...
if ( ! function_exists( 'uds_wp_entry_footer' ) ) {
	...
	function uds_wp_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'uds-wordpress' ) );
			if ( $categories_list && uds_wp_categorized_blog() ) {
				/* translators: %s: Categories of current post */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %s', 'uds-wordpress' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'uds-wordpress' ) );
			if ( $tags_list ) {
				/* translators: %s: Tags of current post */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %s', 'uds-wordpress' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';
			comments_popup_link( esc_html__( 'Leave a comment', 'uds-wordpress' ), esc_html__( '1 Comment', 'uds-wordpress' ), esc_html__( '% Comments', 'uds-wordpress' ) );
			echo '</span>';
		}
		edit_post_link(
			sprintf(
				/* translators: %s: Name of current post */
				esc_html__( 'Edit %s', 'uds-wordpress' ),
